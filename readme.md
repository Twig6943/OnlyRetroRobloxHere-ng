...
This project is missing some small, unimportant things to be a full revival of the launcher. Namely, the TTSHelper binary, as well as the ClientHelper. The ClientHelper binary may come some time in the future, but the TTSHelper binary might not. The reason is these binaries are written in another language that is difficult to decompile, though it assumes it wont be impossible, the effort is better spent on the launcher which is the biggest point. The clients are pretty good all things considered.
